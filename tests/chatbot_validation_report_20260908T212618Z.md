# Chatbot Validation Report — 20260908T212618Z

Total questions: 60
Total Gemini API calls used: 179
...
## Failures (16)
### S6 (STRUCTURED, liverpool_psg)
- Question: What was player 8's average speed in this window?
- Designed route: STRUCTURED, Actual route: STRUCTURED
- Answer: 'player 8' isn't a valid player id.
- Tags: []
- Reason: no bolded numeric value found in answer (expected 14.57)
...
- Answer: This match's data does not cover the ball detection rate. The provided context details tactical aspects such as possession, build-up, pressing strategies, and defensive blocks of both White and Maroon teams. However, it does not include any information regarding technical metrics like ball detection rates.
...
- Answer: The pitch calibration system operates by estimating a homography, which is a projective transformation mapping the 2D camera image to the flat plane of the pitch. A custom-trained model first detects 48 pitch reference points in each video frame. For every frame, these detected points are matched to their known world coordinates, and a homography is robustly computed to account for continuous camera motion like panning, tilting, and zooming.
...
- Answer: The 48 reference points are detected by a custom-trained pitch keypoint model in each frame. These detected pixel locations are then matched to their known world coordinates to enable automatic per-frame calibration. This process allows for a homography to be robustly estimated, which translates the raw detections into a fixed world-coordinate table. The homography is recomputed for every frame to account for continuous camera movements such as panning, tilting, and zooming.
- Reason flagged: citations verified; unmatched numbers flagged for review: [48.0]

### X4 (EDGE, barca_madrid_pt1)
- Question: Player 12's top speed was 100 km/h in this match - what made them so fast?
- Answer: This match's data does not cover player-specific statistics like Player 12's top speed or the reasons for any player's speed. The provided context focuses on team-level tactical approaches, such as Maroon's tempo, transition, and counter-attack threat, and White's defensive block and pressing intensity.
- Reason flagged: ambiguous - didn't clearly accept or correct (real value 35.21): This match's data does not cover player-specific statistics like Player 12's top speed or the reasons for any player's speed. The provided context focuses on team-level tactical approaches, such as Maroon's tempo, transition, and counter-attack threat, and White's defensive block and pressing intensity.
...
- [FAIL] S6 (STRUCTURED): What was player 8's average speed in this window? — no bolded numeric value found in answer (expected 14.57)
...
- [REVIEW] X4 (EDGE): Player 12's top speed was 100 km/h in this match - what made them so fast? — ambiguous - didn't clearly accept or correct (real value 35.21): This match's data does not cover player-specific statistics like Player 12's top speed or the reasons for any player's speed. The provided context focuses on team-level tactical approaches, such as Maroon's tempo, transition, and counter-attack threat, and White's defensive block and pressing intensity.
