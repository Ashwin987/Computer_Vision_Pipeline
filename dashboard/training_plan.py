...
def _prompt_version(func):
    """A short hash of a prompt-generating function's own source code -
    changes automatically the moment that function's prompt wording
    changes, with no separate version number to remember to bump (and
    nothing to forget - the same class of bug that let ai_report silently
    keep serving pre-rewrite text after its prompt was edited: a cached
    plan otherwise has no way to know the prompt that produced it is now
    stale). Every generate_* function below stamps its result with this;
    load_training_plan's caller compares the stored value against a fresh
    call to flag - not silently serve - a plan whose prompt has since
    changed. Hashing the real function source (not a hand-written summary
    of it) is what makes this automatic: any wording edit inside the
    function changes its source, so it can never be forgotten to update
    the way a manual version constant could be."""
    return hashlib.sha256(inspect.getsource(func).encode("utf-8")).hexdigest()[:12]


def is_plan_piece_stale(piece, func):
    """True if `piece` (a team_plan / player_plan / cv_insights dict loaded
    from disk) was generated by an older version of `func`'s prompt than
    the one currently running - i.e. its `_prompt_version` doesn't match a
    fresh call. A piece with no `_prompt_version` at all (saved before this
    safeguard existed) is also treated as stale, not silently trusted."""
    if not piece:
        return False
    return piece.get("_prompt_version") != _prompt_version(func)
        ...
        try:
            ...
            if isinstance(days, list) and len(days) > 0:
                ...
                return {
                    ...
                    "_prompt_version": _prompt_version(generate_team_plan),
                }
        except Exception:
            time.sleep(3)
    return None
        ...
        try:
            ...
            if isinstance(sessions_by_player, dict):
                ...
                return {
                    ...
                    "generated_at": datetime.now(timezone.utc).isoformat(),
                    "_prompt_version": _prompt_version(generate_player_plan),
                }
        except Exception:
            time.sleep(3)
    return None
        ...
        try:
            ...
            if isinstance(result, dict):
                return {
                    ...
                    "generated_at": datetime.now(timezone.utc).isoformat(),
                    "_prompt_version": _prompt_version(generate_cv_insights),
                }
        except Exception:
            time.sleep(3)
    return None
...
# ==========================================
# RENDERING (HTML fragments for st.iframe - this reuses the
